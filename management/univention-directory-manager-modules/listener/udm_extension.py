	...
	if new and not old:
		if listener.configRegistry.get('server/role') == 'domaincontroller_master':
			...
			if cmp_start_vs_50 < 0 and not univentionUCSVersionEnd:
				ud.debug(ud.LISTENER, ud.WARN, '%s: Removing incompatible extension %s (univentionUCSVersionStart='%s' and univentionUCSVersionEnd not set).' % (name, new['cn'][0].decode('UTF-8'), univentionUCSVersionStart))
