...
class TestUniventionUpdater(unittest.TestCase):
    ...
    def test_call_sh_files(self):
        ...
        def structs():
            ...
            struct_r = U.UCSRepoPool5(major=MAJOR, minor=MINOR, part=PART, patchlevel=PATCH, arch=ARCH)
            preup_r = struct_r.path('preup.sh')
            postup_r = struct_r.path('postup.sh')
            struct_c = U.UCSRepoPool5(major=MAJOR, minor=MINOR, part='%s/component' % (PART,), patch='c', arch=ARCH)
